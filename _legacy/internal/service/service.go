...
type ViewModel struct {
	...
	PlayerPos domain.Point
	Player    domain.Character
	Weapon    *domain.Item
...
func (a *App) SetWorldSize(width, height int) {
	a.cfg = domain.Config{Width: width, Height: height}.Normalize()
	...
	if a.mode == ModeSelectSlot && a.sess != nil {
		a.sess = domain.NewSession(a.sess.Seed, a.cfg)
	}
}

// New creates a new game application instance.
func New(storage Storage, seed int64, cfg domain.Config) *App {
	if seed == 0 {
	...
	return &App{
		storage: storage,
		rng:     rng,
		sess:    domain.NewSession(seed, cfg),
		mode:    ModeSelectSlot,
		cfg:     cfg.Normalize(),
	}
}
...
func (a *App) Handle(in Input) (ViewModel, bool, error) {
			...
			case 1:
				if a.selectedHasSave {
					...
					if saved, ok2, err := a.storage.LoadSession(a.activeSlot); err == nil && ok2 {
						a.sess = domain.FromSnapshot(saved.Snapshot)
	...
	case ModeEnterName:
		...
		case InputConfirmYes:
			...
			a.sess = domain.NewSession(seed, a.cfg)
			a.mode = ModePlay
			return a.view(), false, nil
		case InputBackspace:
	...
	case InputHelp:
		a.mode = ModeHelp
	case InputUp:
		a.step(domain.Action{Kind: domain.ActionMove, Dx: 0, Dy: -1})
	case InputDown:
		a.step(domain.Action{Kind: domain.ActionMove, Dx: 0, Dy: 1})
	case InputLeft:
		a.step(domain.Action{Kind: domain.ActionMove, Dx: -1, Dy: 0})
	case InputRight:
		a.step(domain.Action{Kind: domain.ActionMove, Dx: 1, Dy: 0})
	case InputAttack:
		a.step(domain.Action{Kind: domain.ActionAttack})
	case InputInvWeapon:
		a.mode = ModeSelectWeapon
		a.pending = domain.ActionEquipWeapon
	case InputInvFood:
		a.mode = ModeSelectFood
		a.pending = domain.ActionUseFood
	case InputInvElixir:
		a.mode = ModeSelectElixir
		a.pending = domain.ActionUseElixir
	case InputInvScroll:
		a.mode = ModeSelectScroll
		a.pending = domain.ActionUseScroll
	case InputViewStats:
		a.mode = ModeStats
...
func (a *App) step(action domain.Action) {
...
func (a *App) applySelection(idx int) {
	switch a.pending {
	case domain.ActionEquipWeapon:
		if idx == 0 {
			a.step(domain.Action{Kind: domain.ActionUnequipWeapon})
			return
		}
		a.step(domain.Action{Kind: domain.ActionEquipWeapon, Idx: idx - 1})
	case domain.ActionUseFood:
		a.step(domain.Action{Kind: domain.ActionUseFood, Idx: idx - 1})
	case domain.ActionUseElixir:
		a.step(domain.Action{Kind: domain.ActionUseElixir, Idx: idx - 1})
	case domain.ActionUseScroll:
		a.step(domain.Action{Kind: domain.ActionUseScroll, Idx: idx - 1})
	}
}
...
func (a *App) view() ViewModel {
	...
	var wp *domain.Item
	if wid := a.sess.Player.Equipment.Get(domain.SlotWeapon); wid != 0 {
		if it, ok := a.sess.Backpack.FindByID(wid); ok && it.Type == domain.ItemWeapon {
			cp := it
			wp = &cp
		}
	}
	...
	vm := ViewModel{
		...
		Enemies:   append([]domain.Enemy(nil), a.sess.Enemies...),
	...
	switch a.mode {
	case ModeSelectSlot:
		vm.MenuTitle = "Select save slot (1-3). Empty slot starts a new domain."
	...
	case ModeSelectWeapon:
		...
		vm.MenuItems = weaponsMenu(a.sess.Backpack.List(domain.ItemWeapon))
	case ModeSelectFood:
		vm.MenuTitle = "Select food (1-9):"
		vm.MenuItems = itemsMenu(a.sess.Backpack.List(domain.ItemFood))
	case ModeSelectElixir:
		vm.MenuTitle = "Select elixir (1-9):"
		vm.MenuItems = itemsMenu(a.sess.Backpack.List(domain.ItemElixir))
	case ModeSelectScroll:
		vm.MenuTitle = "Select scroll (1-9):"
		vm.MenuItems = itemsMenu(a.sess.Backpack.List(domain.ItemScroll))
	case ModeStats:
...
func itemsMenu(items []domain.Item) []string {
...
func weaponsMenu(items []domain.Item) []string {
	out := make([]string, 0, len(items))
	for i := range items {
...
func statsLines(s domain.Stats) []string {
	return []string{
